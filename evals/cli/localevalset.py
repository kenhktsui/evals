"""
This file defines the `localevalset` CLI for running eval sets.
"""
import argparse
import json
import subprocess
...
def get_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=True,
        help="Exit if any localeval command fails.",
    )
    return parser
...
def run(args, unknown_args, registry: Optional[Registry] = None) -> None:
    ...
    progress = Progress(f"/tmp/localevalset/{args.model}.{args.eval_set}.progress.txt")
